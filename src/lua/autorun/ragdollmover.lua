...
---
-- Create an rgm grab data table.
-- Values:
-- 	axis - The axis that is grabbed.
-- 	axisOffset - The local position from the axis where it is grabbed.
---
function GrabData(axis, axisOffset)
	local data =
	{
		axis = axis,
		axisOffset = axisOffset
...
numpad.Register("rgmAxisChangeState", function(pl, key)
	...
	if pl.rgm.Rotate == nil then
		...
		pl.rgm.Rotate = not pl.rgm.Rotate; -- IT'S A PARADOX
	end
